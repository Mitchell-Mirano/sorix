...
At a high level, the available layers include linear transformations, nonlinear activation functions, normalization modules, and embedding lookup tables. Each class defines the forward computation via the `__call__` interface, while gradient propagation is handled internally through the underlying tensor autograd mechanism. 
...
- **[LeakyReLU Activation](10-LeakyReLU.ipynb)**: ReLU variant with a non-zero slope for negative inputs.
- **[Dropout](06-Dropout.ipynb)**: Training logic for regularization.
- **[Embedding](11-Embedding.ipynb)**: Learnable lookup table mapping categorical indices (tokens, user IDs, product IDs) to dense vectors.
